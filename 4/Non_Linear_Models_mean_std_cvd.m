...
for k = 1:folds
    ...
    if initial_run
        param_matrix = parameters;
        initial_run = false;
    else
        param_matrix = vertcat(param_matrix, parameters);
    end
end

mat_size = size(param_matrix,2);
disp(mat_size);
means = zeros(1,mat_size);
stds = zeros(1,mat_size);
cvds = zeros(1,mat_size);

for i= 1:mat_size
    vec = param_matrix(:,i);
    means(i) = mean(vec);
    stds(i) = std(vec);
    cvds(i) = 100*stds(i) / abs(means(i));
end
